  ...
  try {
    ...
    if (reset) {
      res
        .status(200)
        .redirect(
          `/the-turn/reset/${bcrypt.hashSync(token, 10)}?email=${email}`
        );
    } else {
      ...
      res
        .status(200)
        .redirect(
          `/the-turn/verification-confirmation?verified=true&token=${token}&email=${email}`
        );
    }
  } catch (e) {
...
export default router.handler({
  onError: (err, req, res) => {
    logError("Error verifying user:", err.stack);
